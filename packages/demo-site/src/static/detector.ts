...
function testPlugins() {
  ...
  return {
    ...
    passed,
    value: count,
    explanation: 'Real browsers have plugins, headless browsers have empty array',
    severity: 'CRITICAL',
...
function testWebGLRenderer() {
  try {
    ...
    const passed = !renderer.includes('SwiftShader') && !renderer.includes('Google')
...
function blockAccess(failedTests, score) {
  ...
  sessionStorage.setItem('detectionResults', JSON.stringify(window.detectionResults))
