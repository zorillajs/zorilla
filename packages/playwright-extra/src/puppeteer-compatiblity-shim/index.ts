...
export const isPuppeteerCompat = (obj?: unknown): obj is PlaywrightObject => {
  return (
    !!obj &&
    typeof obj === 'object' &&
    !!(obj as Record<string, unknown>).isCompatShim
  );
};
...
export async function getPageCDPSession(page: pw.Page | pw.Frame) {
  ...
  try {
    ...
    debug('getPageCDPSession: error while creating session:', message);
    debug(
...
export async function getBrowserCDPSession(browser: pw.Browser) {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    debug('getBrowserCDPSession: error while creating session:', message);
    debug(
  ...
  const shim = new Proxy(page, {
    get(target, prop) {
        ...
        return () => ({
          send: async (method: string, params?: Record<string, unknown>) => {
            ...
            return await session.send(
              method as Parameters<pw.CDPSession['send']>[0],
              params
            );
          },
          on: (event: string, listener: (...args: unknown[]) => void) => {
            getPageCDPSession(page).then(session => {
              session.on(event as Parameters<pw.CDPSession['on']>[0], listener);
            });
          },
        });
      }
      ...
      if (prop === 'setUserAgent') {
        return async (userAgent: string, userAgentMetadata?: unknown) => {
          const session = await getPageCDPSession(page);
          return await session.send('Emulation.setUserAgentOverride', {
            userAgent,
            userAgentMetadata: userAgentMetadata as
              | {
                  platform: string;
