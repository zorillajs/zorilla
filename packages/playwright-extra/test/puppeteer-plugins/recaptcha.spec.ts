...
interface RecaptchaPage extends Page {
  findRecaptchas: () => Promise<{ captchas: unknown[]; error?: unknown }>;
  solveRecaptchas: () => Promise<{ solved: unknown[]; error?: unknown }>;
}
...
test('puppeteer-extra-plugin-recaptcha will detect captchas', async ({
  ...
  const pluginErrors = [];
  extraLauncher.plugins.onPluginError = (_plugin, _method, err) => {
  ...
  const { captchas, error } = await (page as RecaptchaPage).findRecaptchas();
...
test('puppeteer-extra-plugin-recaptcha will solve captchas', async ({
  ...
  const { solved, error } = await (page as RecaptchaPage).solveRecaptchas();
  expect(error).toBeFalsy();
