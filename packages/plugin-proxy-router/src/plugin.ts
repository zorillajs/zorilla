...
import { ProxyRouter, type ProxyRouterOpts } from './router.js';

export interface ExtraPluginProxyRouterOptions extends ProxyRouterOpts {
  /**
...
export class ExtraPluginProxyRouter extends PuppeteerExtraPlugin {
  ...
  override get defaults(): ExtraPluginProxyRouterOptions {
    return {
      collectStats: true,
      proxyServerOpts: {
        port: 2800,
  ...
  private get proxyBypassListString() {
    const list = (this.opts as ExtraPluginProxyRouterOptions).proxyBypassList;
    return (list || []).join(',') || undefined;
  }

  override async onPluginRegistered(args?: {
  ...
  override async beforeLaunch(options: unknown = {}): Promise<void> {
