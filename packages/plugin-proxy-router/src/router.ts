...
import { ProxyRouterStats } from './stats.js';
...
export type RouteByHostResponse = ProxyName | undefined;
...
export class ProxyRouter {
  ...
  constructor(opts: ProxyRouterOpts = {}) {
    ...
    this.proxyServer.log = (
      (originalMethod, context) => (connectionId: unknown, str: string) => {
  ...
  public get effectiveProxies(): Record<string, string | null> {
    return {
  ...
  protected async handleProxyServerRequest({
    ...
    connectionId,
    isHttp,
  }: ProxyChain.PrepareRequestFunctionOpts): Promise<
    undefined | ProxyChain.PrepareRequestFunctionResult
  > {
