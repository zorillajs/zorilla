...
import Plugin from '../../../src/evasions/chrome.runtime/index.js';
...
test.skip('vanilla: is chrome false (requires fpcollect)', async () => {
  const pageFn = async page => await page.evaluate(() => window.chrome); // eslint-disable-line
  const { pageFnResult: chrome, hasChrome } =
    await getVanillaFingerPrint(pageFn);
  expect(hasChrome).toBe(false);
  expect(chrome instanceof Object).toBe(false);
  expect(chrome).toBe(undefined);
});

test.skip('stealth: is chrome true (requires fpcollect)', async () => {
  const pageFn = async page => await page.evaluate(() => window.chrome); // eslint-disable-line
  ...
  expect(chrome instanceof Object).toBe(true);
});

test('stealth: will add convincing chrome.runtime object', async () => {
...
test('stealth: will add convincing chrome.runtime.connect', async () => {
  const puppeteer = addExtra(vanillaPuppeteer).use(
    Plugin({
      runOnInsecureOrigins: true, // for testing
    })
...
test('stealth: will add convincing chrome.runtime.connect response', async () => {
  const puppeteer = addExtra(vanillaPuppeteer).use(
    Plugin({
      runOnInsecureOrigins: true, // for testing
    })
...
// test('stealth: error stack is fine', async () => {
//   const puppeteer = addExtra(vanillaPuppeteer).use(
//     Plugin({
//       runOnInsecureOrigins: true // for testing
//     })
